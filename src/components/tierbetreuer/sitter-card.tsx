...
export function SitterCard({ sitter }: { sitter: SitterProfile }) {
  return (
    <Link
      href={`/tierbetreuer/${sitter.id}`}
      aria-label={`${sitter.headline} von ${sitter.profile?.full_name || "Tierbetreuer"} ansehen`}
      className="group block rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2F7A68] focus-visible:ring-offset-2"
    >
      <Card className="grid h-full overflow-hidden border-[#262C36]/10 bg-white shadow-sm transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:shadow-md">
        ...
        <div className="grid gap-4 p-4">
          ...
          <span className="inline-flex items-center gap-1.5 text-sm font-black text-[#D97863]">
            Profil ansehen
